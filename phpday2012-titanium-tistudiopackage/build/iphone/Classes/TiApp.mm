...
- (NSString*)userAgent
{
	if (userAgent==nil)
	{
		...
		NSString *kPhpday2012_DrupaniumUserAgentPrefix = [NSString stringWithFormat:@"%s%s%s %s%s","Appc","eler","ator","Tita","nium"];
		userAgent = [[NSString stringWithFormat:@"%@/%s (%@)",kPhpday2012_DrupaniumUserAgentPrefix,TI_VERSION_STR,currentDeviceInfo] retain];
	}
	return userAgent;
}
